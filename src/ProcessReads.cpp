...
int ProcessBatchReads(KmerIndex &index, const ProgramOptions &opt, MinCollector &tc, std::vector<std::vector<int>> &batchCounts)
{
  ...
  std::cerr << "[quant] finding pseudoalignments for all files ...";
  std::cerr.flush();
...
int ProcessReads(MasterProcessor &MP, const ProgramOptions &opt)
{
  ...
  // for each file
  std::cerr << "[quant] finding pseudoalignments for the reads ...";
  std::cerr.flush();
  ...
  // new added 2025/05/22
  // merge intervals
  auto &intervals = MP.read_intervals;
  std::sort(intervals.begin(), intervals.end());
  std::chrono::microseconds total_us(0);
  if (!intervals.empty())
  {
    auto merged_start = intervals[0].first;
    auto merged_end = intervals[0].second;

    for (size_t i = 1; i < intervals.size(); ++i)
    {
      auto &cur = intervals[i];
      if (cur.first <= merged_end)
      {
        // overlap → extend
        merged_end = std::max(merged_end, cur.second);
      }
      else
      {
        // no overlap → add and reset
        total_us += std::chrono::duration_cast<std::chrono::microseconds>(merged_end - merged_start);
        merged_start = cur.first;
        merged_end = cur.second;
      }
    }
    // add last segment
    total_us += std::chrono::duration_cast<std::chrono::microseconds>(merged_end - merged_start);
  }
  std::cout << "\n[summary] FASTQ load time: " << (total_us.count() / 1'000'000.0) << " s" << std::endl;
  std::cout <<   "[summary] Pseudo-alignment time: " << duration.count() - (total_us.count() / 1'000'000.0) << " s" << std::endl;
  std::cout <<   "[summary] Total runtime: " << duration.count() << " s" << std::endl;
  // new added 2025/05/22
...
void MasterProcessor::processAln(const EMAlgorithm &em, bool useEM = true)
{
  ...
  std::cerr << "[  bam] writing pseudoalignments to BAM format .. ";
  std::cerr.flush();
  ...
  for (auto &chrw : chrWeights)
  {
    ...
    std::sort(chrw.begin(), chrw.end(), [](std::pair<uint32_t, uint32_t> a, std::pair<uint32_t, uint32_t> b)
  ...
  */
  ...
  if (opt.genomebam)
  {
    std::cerr << "[  bam] sorting BAM files .. ";
    std::cerr.flush();
    for (int i = 0; i < numSortFiles; i++)
    {
      sam_close(bamfps[i]);
    ...
    for (int i = 0; i < numSortFiles; i++)
    {
      ...
      if (i < numSortFiles - 1)
      {
        ...
        std::sort(bb.begin(), bb.end(), [](std::pair<uint64_t, uint64_t> a, std::pair<uint64_t, uint64_t> b)
                  { return a.first < b.first; });
    ...
    std::cerr << "[  bam] indexing BAM file .. ";
    std::cerr.flush();
...
ReadProcessor::ReadProcessor(const KmerIndex &index, const ProgramOptions &opt, const MinCollector &tc, MasterProcessor &mp, int _id) : paired(!opt.single_end), tc(tc), index(index), mp(mp), id(_id)
{
  // initialize buffer
  bufsize = mp.bufsize;
...
ReadProcessor::ReadProcessor(ReadProcessor &&o) : paired(o.paired),
                                                  tc(o.tc),
                                                  index(o.index),
                                                  mp(o.mp),
                                                  id(o.id),
...
void ReadProcessor::operator()()
{
  while (true)
  {
    ...
    {
      std::lock_guard<std::mutex> lock(mp.read_time_mutex);
      mp.read_intervals.emplace_back(t_read_start, t_read_end);
    }
...
AlnProcessor::AlnProcessor(const KmerIndex &index, const ProgramOptions &opt, MasterProcessor &mp, const EMAlgorithm &em, const Transcriptome &model, bool useEM, int _id) : paired(!opt.single_end), index(index), mp(mp), em(em), model(model), useEM(useEM), id(_id)
{
  // initialize buffer
  bufsize = mp.bufsize;
  buffer = new char[bufsize];
...
AlnProcessor::AlnProcessor(AlnProcessor &&o) : paired(o.paired),
                                               index(o.index),
                                               em(o.em),
                                               mp(o.mp),
                                               model(o.model),
...
SequenceReader::SequenceReader(SequenceReader &&o) : fp1(o.fp1),
                                                     fp2(o.fp2),
                                                     seq1(o.seq1),
                                                     seq2(o.seq2),
                                                     l1(o.l1),
