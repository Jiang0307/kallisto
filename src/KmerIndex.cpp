...
void KmerIndex::BuildTranscripts(const ProgramOptions &opt)
{
  ...
  for (auto &fasta : opt.transfasta)
  {
    ...
    while (true)
    {
      ...
      if (unique_names.find(name) != unique_names.end())
      {
        ...
        {
          for (int i = 1;; i++)
          { // potential bug if you have more than 2^32 repeated names
  ...
  if (polyAcount > 0)
  {
    std::cerr << "[build] warning: clipped off poly-A tail (longer than 10)" << std::endl
              << "        from " << polyAcount << " target sequences" << std::endl;
  }

  if (countNonNucl > 0)
  {
    std::cerr << "[build] warning: replaced " << countNonNucl << " non-ACGUT characters in the input sequence" << std::endl
              << "        with pseudorandom nucleotides" << std::endl;
  }
  if (countUNuc > 0)
  {
...
void KmerIndex::BuildDeBruijnGraph(const ProgramOptions &opt, const std::vector<std::string> &seqs)
{
  ...
  std::cerr << "[build] counting k-mers ... ";
  std::cerr.flush();
  // gather all k-mers
  for (int i = 0; i < seqs.size(); i++)
  {
  ...
  std::cerr << "[build] building target de Bruijn graph ... ";
  std::cerr.flush();
...
{
  std::cerr << "[build] creating equivalence classes ... ";
  std::cerr.flush();
